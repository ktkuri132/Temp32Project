...
// 初始化IIC
void Soft_IIC_Init(df_soft_iic_t *i2c_dev)
{
    if (i2c_dev->gpio_init != NULL)
    {
        i2c_dev->gpio_init(); // 初始化IIC的GPIO端口
    }
    i2c_dev->scl(1);
    i2c_dev->sda(1);
    i2c_dev->init_flag = true;
}

// 产生IIC起始信号
void Soft_IIC_Start(df_soft_iic_t *i2c_dev)
{
    i2c_dev->sda_out(); // sda线输出
    i2c_dev->sda(1);
    i2c_dev->scl(1);
    // i2c_dev->delay_us(4);
    i2c_dev->sda(
        0);          // START:when CLK is high,DATA change form high to low
                     //    i2c_dev->delay_us(1);
    i2c_dev->scl(0); // 钳住I2C总线，准备发送或接收数据
}
// 产生IIC停止信号
void Soft_IIC_Stop(df_soft_iic_t *i2c_dev)
{
    i2c_dev->sda_out(); // sda线输出
    i2c_dev->scl(0);
    i2c_dev->sda(
    ...
    // delay_us(4);
    i2c_dev->scl(1);
    i2c_dev->sda(1); // 发送I2C总线结束信号
    // delay_us(4);
}
// 等待应答信号到来
// 返回值：1，接收应答失败
//         0，接收应答成功
uint8_t Soft_IIC_Wait_Ack(df_soft_iic_t *i2c_dev)
{
    uint8_t ucErrTime = 0;
    i2c_dev->sda_in(); // SDA设置为输入
    i2c_dev->sda(1);
    //    i2c_dev->delay_us(1);
    i2c_dev->scl(1);
    //    i2c_dev->delay_us(1);
    while (i2c_dev->read_sda())
    {
        ucErrTime++;
        if (ucErrTime > 250)
        {
...
// 产生ACK应答
void Soft_IIC_Ack(df_soft_iic_t *i2c_dev)
{
    i2c_dev->scl(0);
    i2c_dev->sda_out();
    i2c_dev->sda(0);
...
// 不产生ACK应答
void Soft_IIC_NAck(df_soft_iic_t *i2c_dev)
{
    i2c_dev->scl(0);
    i2c_dev->sda_out();
    i2c_dev->sda(1);
...
// 1，有应答
// 0，无应答
void Soft_IIC_Send_Byte(df_soft_iic_t *i2c_dev, uint8_t txd)
{
    uint8_t t;
    i2c_dev->sda_out();
    i2c_dev->scl(0); // 拉低时钟开始数据传输
    for (t = 0; t < 8; t++)
    {
        i2c_dev->sda((txd & 0x80) >> 7);
        txd <<= 1;
        //    i2c_dev->delay_us(1);
        i2c_dev->scl(1);
        //    i2c_dev->delay_us(1);
...
uint8_t Soft_IIC_Receive_Byte(df_soft_iic_t *i2c_dev, unsigned char ack)
{
...
uint8_t Soft_IIC_Read_Byte(df_soft_iic_t *i2c_dev, uint8_t addr, uint8_t reg)
{
    uint8_t res;
    Soft_IIC_Start(i2c_dev);
    Soft_IIC_Send_Byte(i2c_dev, addr | 0); // 发送器件地址+写命令
