 ...
 *
 *          设计思想：
 *          1. 中断服务程序(ISR)中只做最少的工作：标记中断状态、保存数据
 *          2. 实际的业务处理在主循环中按优先级顺序执行
 *          3. 通过状态机机制避免中断处理冲突
 *
 *          工作流程：
 *          ┌─────────────┐    ┌──────────────────┐    ┌─────────────────┐
 *          │  硬件中断   │───>│ irq_handle_loader │───>│ 标记PENDING状态 │
 ...
 *
 *          状态转换：
 *          DISABLE ──(loader)──> PENDING ──(runner)──> READY ──(执行后)──> DISABLE
 *
 * @version 1.0
 * @date    2025-12-27
 *
 * @note    使用须知：
 *          - 中断句柄数组必须以 irq_num = 0xFFFF 作为结束标记
 ...
 *
 *          注意事项：
 *          - 当前实现中 sizeof(irq_handles) 无法正确获取数组大小
 *            因为数组作为参数传递时会退化为指针
 *          - 建议改用结束标记(0xFFFF)来判断数组边界
 *
...
/**
 * @brief   中断处理加载器 - 在ISR中调用，用于加载中断数据
 *
 * @details 此函数应在硬件中断服务程序(ISR)中调用。
 *          主要功能：
 *          1. 根据中断号查找对应的处理句柄
 ...
 * @retval  -1  加载失败，上次中断未处理完，本次数据被丢弃
 * @retval  -2  未找到对应的中断处理句柄
 *
 * @warning 此函数运行在中断上下文中，应尽量简短
...
int irq_handle_loader(irq_handle_t ih[], uint16_t irq_num, void *argv[])
{
    ...
    if (irq_handle_index >= 0)
    {
        ...
        {
            /* 步骤3：标记中断状态为待处理 */
            ih[irq_handle_index].irq_state = PENDING;
...
/**
 * @brief   中断处理运行器 - 在主循环中调用，执行中断回调函数
 *
 ...
 *
 * @return  int 执行结果
 * @retval  0   成功执行了一个中断回调
 * @retval  -1  没有待处理的中断或内存分配失败
 *
...
int irq_handle_runner(irq_handle_t ih[])
{
    ...
    {
        if (ih[i].irq_state == PENDING) /* 只处理PENDING状态 */
        {
            /* 以优先级为索引存入临时数组，存入结构体的地址 */
            ih_temp[ih[i].irq_priority] = ih[i];
            /* 将状态从PENDING改为READY，表示即将执行 */
            ih_temp[ih[i].irq_priority].irq_state = READY;
        }
        ih[i].irq_state = DISABLE; /* 清除原句柄状态 */
    }
    /*
     * 第二阶段：按优先级顺序（从0开始）查找并执行
     * 优先级0最高，先执行
     */
    for (; j < IRQ_HANDLE_MAX_NUM; j++)
    {
        /* 检查该优先级是否有待处理的中断 */
        if (ih_temp[j].irq_state == READY)
        {
            /* 执行中断回调函数，传入参数 */
