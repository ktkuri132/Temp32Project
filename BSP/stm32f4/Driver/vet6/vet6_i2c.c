...
/* I2C外设基地址表 */
static I2C_TypeDef *const i2c_base_table[VET6_I2C_MAX] = {
    I2C1, I2C2, I2C3};
...
int vet6_i2c_pins_init(vet6_i2c_pinmap_t pinmap)
{
    switch (pinmap)
    {
    case VET6_I2C1_PINS_PB6_PB7:
        /* I2C1: PB6(SCL), PB7(SDA) - AF4, 开漏输出 */
        vet6_gpio_config_af_otype(VET6_PB6, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        vet6_gpio_config_af_otype(VET6_PB7, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        break;

    case VET6_I2C1_PINS_PB8_PB9:
        /* I2C1: PB8(SCL), PB9(SDA) - AF4, 开漏输出 */
        vet6_gpio_config_af_otype(VET6_PB8, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        vet6_gpio_config_af_otype(VET6_PB9, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        break;

    case VET6_I2C2_PINS_PB10_PB11:
        /* I2C2: PB10(SCL), PB11(SDA) - AF4, 开漏输出 */
        vet6_gpio_config_af_otype(VET6_PB10, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        vet6_gpio_config_af_otype(VET6_PB11, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        break;

    case VET6_I2C3_PINS_PA8_PC9:
        /* I2C3: PA8(SCL), PC9(SDA) - AF4, 开漏输出 */
        vet6_gpio_config_af_otype(VET6_PA8, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        vet6_gpio_config_af_otype(VET6_PC9, VET6_GPIO_AF4_I2C1_3, VET6_GPIO_OTYPE_OD);
        break;
...
int vet6_i2c_init(vet6_i2c_handle_t *handle, const vet6_i2c_config_t *config)
{
    ...
    /* 使能时钟 */
    vet6_i2c_clk_enable(config->i2c);

    /* 初始化引脚 */
    if (vet6_i2c_pins_init(config->pinmap) != 0)
    {
        return -1;
    }
    ...
    /* 配置时钟频率 */
    uint32_t pclk1 = VET6_I2C_APB1_CLK;
...
{
    vet6_i2c_config_t config = {
        .i2c = i2c,
        .speed = speed,
        .addr_mode = VET6_I2C_ADDR_7BIT,
        .pinmap = pinmap,
        .use_dma = false};

    return vet6_i2c_init(handle, &config);
}
...
void vet6_i2c_deinit(vet6_i2c_handle_t *handle)
{
    ...
    /* 禁用时钟 */
    vet6_i2c_clk_disable(handle->config.i2c);
...
int vet6_i2c_start(vet6_i2c_handle_t *handle)
{
    ...
    /* 等待总线空闲 */
    if (i2c_wait_flag_sr2(i2c, I2C_SR2_BUSY, 0, VET6_I2C_TIMEOUT) != 0)
    {
        return -1;
    }
    ...
    /* 等待SB标志 */
    if (i2c_wait_flag(i2c, I2C_SR1_SB, 1, VET6_I2C_TIMEOUT) != 0)
    {
        return -1;
    }
...
int vet6_i2c_send_addr(vet6_i2c_handle_t *handle, uint8_t addr, uint8_t dir)
{
    ...
    /* 等待ADDR标志 */
    if (i2c_wait_flag(i2c, I2C_SR1_ADDR, 1, VET6_I2C_TIMEOUT) != 0)
    {
        /* 检查是否是NACK */
        if (i2c->SR1 & I2C_SR1_AF)
        {
            i2c->SR1 &= ~I2C_SR1_AF;
            vet6_i2c_stop(handle);
            return -1;
        }
        return -1;
    }
...
int vet6_i2c_send_byte(vet6_i2c_handle_t *handle, uint8_t data)
{
    ...
    /* 等待TXE */
    if (i2c_wait_flag(i2c, I2C_SR1_TXE, 1, VET6_I2C_TIMEOUT) != 0)
    {
        return -1;
    }
    ...
    /* 等待BTF */
    if (i2c_wait_flag(i2c, I2C_SR1_BTF, 1, VET6_I2C_TIMEOUT) != 0)
    {
        return -1;
    }
...
int vet6_i2c_receive_byte_ack(vet6_i2c_handle_t *handle, uint8_t *data)
{
    ...
    /* 等待RXNE */
    if (i2c_wait_flag(i2c, I2C_SR1_RXNE, 1, VET6_I2C_TIMEOUT) != 0)
    {
        return -1;
    }
                   ...
                   const uint8_t *data, uint16_t len)
{
    ...
    /* 发送起始条件 */
    if (vet6_i2c_start(handle) != 0)
        return -1;

    /* 发送设备地址+写 */
    if (vet6_i2c_send_addr(handle, dev_addr, 0) != 0)
        return -1;

    /* 发送数据 */
    for (uint16_t i = 0; i < len; i++)
    {
        if (vet6_i2c_send_byte(handle, data[i]) != 0)
        {
            vet6_i2c_stop(handle);
            return -1;
        }
                  ...
                  uint8_t *data, uint16_t len)
{
    ...
    /* 发送设备地址+读 */
    if (vet6_i2c_send_addr(handle, dev_addr, 1) != 0)
        return -1;
    ...
    for (uint16_t i = 0; i < len; i++)
    {
        ...
        {
            /* 最后一个字节发送NACK */
            if (vet6_i2c_receive_byte_nack(handle, &data[i]) != 0)
            {
                vet6_i2c_stop(handle);
                return -1;
            }
        ...
        {
            if (vet6_i2c_receive_byte_ack(handle, &data[i]) != 0)
            {
                vet6_i2c_stop(handle);
                return -1;
            }
                       ...
                       uint8_t reg_addr, const uint8_t *data, uint16_t len)
{
    ...
    /* 发送寄存器地址 */
    if (vet6_i2c_send_byte(handle, reg_addr) != 0)
    {
        vet6_i2c_stop(handle);
        return -1;
    }
...
bool vet6_i2c_device_ready(vet6_i2c_handle_t *handle, uint8_t dev_addr)
{
    ...
    /* 发送设备地址+写 */
    int ret = vet6_i2c_send_addr(handle, dev_addr, 0);
...
uint8_t vet6_i2c_scan(vet6_i2c_handle_t *handle, uint8_t *addr_list, uint8_t max_count)
{
    ...
    {
        if (vet6_i2c_device_ready(handle, addr))
        {
...
void vet6_i2c_reset(vet6_i2c_handle_t *handle)
{
    ...
    /* 重新初始化 */
    vet6_i2c_init(handle, &handle->config);
}
...
/* 软件I2C延时 */
static void soft_i2c_delay(vet6_soft_i2c_handle_t *handle)
{
    delay_us(handle->delay_us);
}

/* 设置SCL */
static void soft_i2c_scl(vet6_soft_i2c_handle_t *handle, uint8_t state)
{
    vet6_gpio_write(handle->scl_pin, state);
}

/* 设置SDA */
static void soft_i2c_sda(vet6_soft_i2c_handle_t *handle, uint8_t state)
{
    vet6_gpio_write(handle->sda_pin, state);
}

/* 读取SDA */
static uint8_t soft_i2c_read_sda(vet6_soft_i2c_handle_t *handle)
{
    return vet6_gpio_read(handle->sda_pin);
}

/* SDA设置为输入 */
static void soft_i2c_sda_input(vet6_soft_i2c_handle_t *handle)
{
    vet6_gpio_config_input_pu(handle->sda_pin);
}

/* SDA设置为输出 */
static void soft_i2c_sda_output(vet6_soft_i2c_handle_t *handle)
{
    vet6_gpio_config_output_od(handle->sda_pin);
}
...
int vet6_soft_i2c_init(vet6_soft_i2c_handle_t *handle, const vet6_soft_i2c_config_t *config)
{
    ...
    /* 配置GPIO为开漏输出 */
    vet6_gpio_config_output_od(handle->scl_pin);
    vet6_gpio_config_output_od(handle->sda_pin);

    /* 默认空闲状态:都为高电平 */
    vet6_gpio_set(handle->scl_pin);
    vet6_gpio_set(handle->sda_pin);
...
{
    vet6_soft_i2c_config_t config = {
        .scl_pin = scl_pin,
        .sda_pin = sda_pin,
        .delay_us = 2};

    return vet6_soft_i2c_init(handle, &config);
}
                            ...
                            uint8_t reg_addr, const uint8_t *data, uint16_t len)
{
    ...
    vet6_soft_i2c_start(handle);

    /* 发送设备地址+写 */
    if (vet6_soft_i2c_send_byte(handle, (dev_addr << 1) | 0) != 0)
    {
        vet6_soft_i2c_stop(handle);
        return -1;
    }

    /* 发送寄存器地址 */
    if (vet6_soft_i2c_send_byte(handle, reg_addr) != 0)
    {
        vet6_soft_i2c_stop(handle);
        return -1;
    }
    ...
    if (data != NULL && len > 0)
    {
        for (uint16_t i = 0; i < len; i++)
        {
            if (vet6_soft_i2c_send_byte(handle, data[i]) != 0)
            {
                vet6_soft_i2c_stop(handle);
                return -1;
            }
                           ...
                           uint8_t reg_addr, uint8_t *data, uint16_t len)
{
    ...
    /* 发送设备地址+读 */
    if (vet6_soft_i2c_send_byte(handle, (dev_addr << 1) | 1) != 0)
    {
        vet6_soft_i2c_stop(handle);
        return -1;
    }
    ...
    for (uint16_t i = 0; i < len; i++)
    {
        data[i] = vet6_soft_i2c_receive_byte(handle, (i < len - 1));
    }
...
bool vet6_soft_i2c_device_ready(vet6_soft_i2c_handle_t *handle, uint8_t dev_addr)
{
    ...
    uint8_t ack = vet6_soft_i2c_send_byte(handle, (dev_addr << 1) | 0);
