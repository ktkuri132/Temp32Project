 ...
 */
void SH1106_W_SCL(uint8_t BitValue)
{
 ...
 */
void SH1106_W_SDA(uint8_t BitValue)
{
 ...
 * 返 回 值：无
 */
void SH1106_I2C_Start(void)
{
    SH1106_W_SDA(1); // 释放SDA，确保SDA为高电平
    SH1106_W_SCL(1); // 释放SCL，确保SCL为高电平
    SH1106_W_SDA(0); // 在SCL高电平期间，拉低SDA，产生起始信号
    SH1106_W_SCL(0); // 起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
}
 ...
 * 返 回 值：无
 */
void SH1106_I2C_Stop(void)
{
    SH1106_W_SDA(0); // 拉低SDA，确保SDA为低电平
    SH1106_W_SCL(1); // 释放SCL，使SCL呈现高电平
    SH1106_W_SDA(1); // 在SCL高电平期间，释放SDA，产生终止信号
}
 ...
 * 返 回 值：无
 */
void SH1106_I2C_SendByte(uint8_t Byte)
{
    uint8_t i;
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        /*两个!的作用是，让所有非零的值变为1*/
        SH1106_W_SDA(!!(Byte & (0x80 >> i)));
        SH1106_W_SCL(1); // 释放SCL，从机在SCL高电平期间读取SDA
        SH1106_W_SCL(0); // 拉低SCL，主机开始发送下一位数据
    }

    SH1106_W_SCL(1); // 额外的一个时钟，不处理应答信号
    SH1106_W_SCL(0);
}
...
{
    SH1106_I2C_Start();           // I2C起始
    SH1106_I2C_SendByte(0x78);    // 发送SH1106的I2C从机地址
    SH1106_I2C_SendByte(0x00);    // 控制字节，给0x00，表示即将写命令
    SH1106_I2C_SendByte(Command); // 写入指定的命令
    SH1106_I2C_Stop();            // I2C终止
}
...
void SH1106_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    SH1106_I2C_Start();        // I2C起始
    SH1106_I2C_SendByte(0x78); // 发送SH1106的I2C从机地址
    SH1106_I2C_SendByte(0x40); // 控制字节，给0x40，表示即将写数据
    /*循环Count次，进行连续的数据写入*/
    for (i = 0; i < Count; i++)
    {
        SH1106_I2C_SendByte(Data[i]); // 依次发送Data的每一个数据
    }
    SH1106_I2C_Stop(); // I2C终止
}
 ...
 * 返 回 值：等于X的Y次方
 */
uint32_t SH1106_Pow(uint32_t X, uint32_t Y)
{
 ...
 */
uint8_t SH1106_pnpoly(uint8_t nvert, int16_t *vertx, int16_t *verty, int16_t testx, int16_t testy)
{
    int16_t i, j, c = 0;
 ...
 */
uint8_t SH1106_IsInAngle(int16_t X, int16_t Y, int16_t StartAngle, int16_t EndAngle)
{
    int16_t PointAngle;
 ...
 * 参    数：FontSize 指定字体大小
 *           范围：SH1106_8X16		宽8像素，高16像素
 *                 SH1106_6X8		宽6像素，高8像素
 * 返 回 值：无
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SH1106_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
    if (FontSize == SH1106_8X16) // 字体为宽8像素，高16像素
    {
        /*将ASCII字模库SH1106_F8x16的指定数据以8*16的图像格式显示*/
        SH1106_ShowImage(X, Y, 8, 16, SH1106_F8x16[Char - ' ']);
    }
    else if (FontSize == SH1106_6X8) // 字体为宽6像素，高8像素
    {
        /*将ASCII字模库SH1106_F6x8的指定数据以6*8的图像格式显示*/
        SH1106_ShowImage(X, Y, 6, 8, SH1106_F6x8[Char - ' ']);
    }
}

/**
 * 函    数：SH1106显示字符串（支持ASCII码和中文混合写入）
 ...
 *                 SH1106_6X8		宽6像素，高8像素
 * 返 回 值：无
 * 说    明：显示的中文字符需要在SH1106_Data.c里的SH1106_CF16x16数组定义
 *           未找到指定中文字符时，会显示默认图形（一个方框，内部一个问号）
 *           当字体大小为SH1106_8X16时，中文字符以16*16点阵正常显示
 *           当字体大小为SH1106_6X8时，中文字符以6*8点阵显示'?'
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SH1106_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
    ...
    {
#ifdef SH1106_CHARSET_UTF8 // 定义字符集为UTF8
...
#ifdef SH1106_CHARSET_GB2312 // 定义字符集为GB2312
        ...
        {
            /*使用SH1106_ShowChar显示此字符*/
            SH1106_ShowChar(X + XOffset, Y, SingleChar[0], FontSize);
            XOffset += FontSize;
        }
        else // 否则，即多字节字符
        {
            ...
            for (pIndex = 0; strcmp(SH1106_CF16x16[pIndex].Index, "") != 0; pIndex++)
            {
                /*找到匹配的字符*/
                if (strcmp(SH1106_CF16x16[pIndex].Index, SingleChar) == 0)
                {
                    break; // 跳出循环，此时pIndex的值为指定字符的索引
                }
            }
            if (FontSize == SH1106_8X16) // 给定字体为8*16点阵
            {
                /*将字模库SH1106_CF16x16的指定数据以16*16的图像格式显示*/
                SH1106_ShowImage(X + XOffset, Y, 16, 16, SH1106_CF16x16[pIndex].Data);
                XOffset += 16;
            }
            else if (FontSize == SH1106_6X8) // 给定字体为6*8点阵
            {
                /*空间不足，此位置显示'?'*/
                SH1106_ShowChar(X + XOffset, Y, '?', SH1106_6X8);
                XOffset += SH1106_6X8;
            }
        }
    }
...
void SH1106_ShowNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    {
        /*调用SH1106_ShowChar函数，依次显示每个数字*/
        /*Number / SH1106_Pow(10, Length - i - 1) % 10 可以十进制提取数字的每一位*/
        /*+ '0' 可将数字转换为字符格式*/
        SH1106_ShowChar(X + i * FontSize, Y, Number / SH1106_Pow(10, Length - i - 1) % 10 + '0',
                        FontSize);
    }
}

/**
 * 函    数：SH1106显示有符号数字（十进制，整数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
...
void SH1106_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    if (Number >= 0) // 数字大于等于0
    {
        SH1106_ShowChar(X, Y, '+', FontSize); // 显示+号
    ...
    {
        /*调用SH1106_ShowChar函数，依次显示每个数字*/
        /*Number1 / SH1106_Pow(10, Length - i - 1) % 10 可以十进制提取数字的每一位*/
        /*+ '0' 可将数字转换为字符格式*/
        SH1106_ShowChar(X + (i + 1) * FontSize, Y, Number1 / SH1106_Pow(10, Length - i - 1) % 10 + '0',
                        FontSize);
    }
}

/**
 * 函    数：SH1106显示十六进制数字（十六进制，正整数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
...
void SH1106_ShowHexNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    {
        /*以十六进制提取数字的每一位*/
        SingleNumber = Number / SH1106_Pow(16, Length - i - 1) % 16;

        if (SingleNumber < 10) // 单个数字小于10
        {
            /*调用SH1106_ShowChar函数，显示此数字*/
            /*+ '0' 可将数字转换为字符格式*/
            SH1106_ShowChar(X + i * FontSize, Y, SingleNumber + '0', FontSize);
        }
        else // 单个数字大于10
        {
            /*调用SH1106_ShowChar函数，显示此数字*/
            /*+ 'A' 可将数字转换为从A开始的十六进制字符*/
            SH1106_ShowChar(X + i * FontSize, Y, SingleNumber - 10 + 'A', FontSize);
        }
    }
}

/**
 * 函    数：SH1106显示二进制数字（二进制，正整数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
 * 参    数：Number 指定要显示的数字，范围：0x00000000~0xFFFFFFFF
...
void SH1106_ShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    {
        /*调用SH1106_ShowChar函数，依次显示每个数字*/
        /*Number / SH1106_Pow(2, Length - i - 1) % 2 可以二进制提取数字的每一位*/
        /*+ '0' 可将数字转换为字符格式*/
        SH1106_ShowChar(X + i * FontSize, Y, Number / SH1106_Pow(2, Length - i - 1) % 2 + '0',
                        FontSize);
    }
}

/**
 * 函    数：SH1106显示浮点数字（十进制，小数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SH1106_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength,
                         uint8_t FontSize)
{
    ...
    PowNum = SH1106_Pow(10, FraLength); // 根据指定小数的位数，确定乘数
    FraNum = round(Number * PowNum);    // 将小数乘到整数，同时四舍五入，避免显示误差
    IntNum += FraNum / PowNum;          // 若四舍五入造成了进位，则需要再加给整数

    /*显示整数部分*/
    SH1106_ShowNum(X + FontSize, Y, IntNum, IntLength, FontSize);

    /*显示小数点*/
    SH1106_ShowChar(X + (IntLength + 1) * FontSize, Y, '.', FontSize);

    /*显示小数部分*/
    SH1106_ShowNum(X + (IntLength + 2) * FontSize, Y, FraNum, FraLength, FontSize);
}
...
/**
 * 函    数：SH1106使用printf函数打印格式化字符串（支持ASCII码和中文混合写入）
...
void SH1106_Printf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
    ...
    va_end(arg);                   // 结束变量arg
    // 清除行尾空格
    SH1106_ShowString(X, Y, String, FontSize); // SH1106显示字符数组（字符串）
}
...
void SH1106_DrawRectangle(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, uint8_t IsFilled)
{
    ...
    if (!IsFilled) // 指定矩形不填充
    {
        ...
        for (i = X; i < X + Width; i++)
        {
            SH1106_DrawPoint(i, Y);
            SH1106_DrawPoint(i, Y + Height - 1);
        }
        /*遍历左右Y坐标，画矩形左右两条线*/
        for (i = Y; i < Y + Height; i++)
        {
            SH1106_DrawPoint(X, i);
            SH1106_DrawPoint(X + Width - 1, i);
        }
    }
    else // 指定矩形填充
    {
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SH1106_DrawTriangle(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1, int16_t X2, int16_t Y2,
                         uint8_t IsFilled)
{
    ...
    if (!IsFilled) // 指定三角形不填充
    {
        ...
        SH1106_DrawLine(X0, Y0, X2, Y2);
        SH1106_DrawLine(X1, Y1, X2, Y2);
    }
    else // 指定三角形填充
    {
        /*找到三个点最小的X、Y坐标*/
        ...
        for (i = minx; i <= maxx; i++)
        {
            ...
            for (j = miny; j <= maxy; j++)
            {
                /*调用SH1106_pnpoly，判断指定点是否在指定三角形之中*/
                /*如果在，则画点，如果不在，则不做处理*/
                if (SH1106_pnpoly(3, vx, vy, i, j))
                {
                    SH1106_DrawPoint(i, j);
                }
            }
...
void SH1106_DrawCircle(int16_t X, int16_t Y, uint8_t Radius, uint8_t IsFilled)
{
    ...
    /*画每个八分之一圆弧的起始点*/
    SH1106_DrawPoint(X + x, Y + y);
    SH1106_DrawPoint(X - x, Y - y);
    SH1106_DrawPoint(X + y, Y + x);
    SH1106_DrawPoint(X - y, Y - x);
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SH1106_DrawArc(int16_t X, int16_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle,
                    uint8_t IsFilled)
{
    int16_t x, y, d, j;
    ...
    if (SH1106_IsInAngle(x, y, StartAngle, EndAngle))
    {
        SH1106_DrawPoint(X + x, Y + y);
    }
    if (SH1106_IsInAngle(-x, -y, StartAngle, EndAngle))
    ...
    if (SH1106_IsInAngle(y, x, StartAngle, EndAngle))
    {
        SH1106_DrawPoint(X + y, Y + x);
    }
    if (SH1106_IsInAngle(-y, -x, StartAngle, EndAngle))
    ...
    if (IsFilled) // 指定圆弧填充
    {
        ...
        for (j = -y; j < y; j++)
        {
            ...
            if (SH1106_IsInAngle(0, j, StartAngle, EndAngle))
            {
                SH1106_DrawPoint(X, Y + j);
            }
        }
    ...
    while (x < y) // 遍历X轴的每个点
    {
        ...
        if (IsFilled) // 指定圆弧填充
        {
            ...
            {
                /*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
                if (SH1106_IsInAngle(x, j, StartAngle, EndAngle))
                {
                    SH1106_DrawPoint(X + x, Y + j);
                }
                if (SH1106_IsInAngle(-x, j, StartAngle, EndAngle))
            ...
            {
                /*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
                if (SH1106_IsInAngle(-y, j, StartAngle, EndAngle))
                {
                    SH1106_DrawPoint(X - y, Y + j);
                }
                if (SH1106_IsInAngle(y, j, StartAngle, EndAngle))
