 ...
 */
void SSD1306_W_SCL(uint8_t BitValue)
{
 ...
 */
void SSD1306_W_SDA(uint8_t BitValue)
{
...
{
    SSD1306_W_SDA(1); // 释放SDA，确保SDA为高电平
    SSD1306_W_SCL(1); // 释放SCL，确保SCL为高电平
    SSD1306_W_SDA(0); // 在SCL高电平期间，拉低SDA，产生起始信号
    SSD1306_W_SCL(0); // 起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
}
...
{
    SSD1306_W_SDA(0); // 拉低SDA，确保SDA为低电平
    SSD1306_W_SCL(1); // 释放SCL，使SCL呈现高电平
    SSD1306_W_SDA(1); // 在SCL高电平期间，释放SDA，产生终止信号
}
...
void SSD1306_I2C_SendByte(uint8_t Byte)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        SSD1306_W_SCL(1); // 释放SCL，从机在SCL高电平期间读取SDA
        SSD1306_W_SCL(0); // 拉低SCL，主机开始发送下一位数据
    }
...
{
    SSD1306_I2C_Start();           // I2C起始
    SSD1306_I2C_SendByte(0x78);    // 发送SSD1306的I2C从机地址
    SSD1306_I2C_SendByte(0x00);    // 控制字节，给0x00，表示即将写命令
    SSD1306_I2C_SendByte(Command); // 写入指定的命令
    SSD1306_I2C_Stop();            // I2C终止
}
 ...
 * 返 回 值：无
 */
void SSD1306_WriteData(uint8_t *Data, uint8_t Count)
{
    uint8_t i;

    SSD1306_I2C_Start();        // I2C起始
    SSD1306_I2C_SendByte(0x78); // 发送SSD1306的I2C从机地址
    SSD1306_I2C_SendByte(0x40); // 控制字节，给0x40，表示即将写数据
    /*循环Count次，进行连续的数据写入*/
    for (i = 0; i < Count; i++)
    {
        SSD1306_I2C_SendByte(Data[i]); // 依次发送Data的每一个数据
    }
    SSD1306_I2C_Stop(); // I2C终止
}
 ...
 */
void SSD1306_W_D0(uint8_t BitValue)
{
 ...
 */
void SSD1306_W_D1(uint8_t BitValue)
{
 ...
 */
void SSD1306_W_RES(uint8_t BitValue)
{
 ...
 */
void SSD1306_W_DC(uint8_t BitValue)
{
 ...
 */
void SSD1306_W_CS(uint8_t BitValue)
{
 ...
 * 返 回 值：无
 */
void SSD1306_SPI_SendByte(uint8_t Byte)
{
    uint8_t i;
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        /*两个!的作用是，让所有非零的值变为1*/
        SSD1306_W_D1(!!(Byte & (0x80 >> i)));
        SSD1306_W_D0(1); // 拉高D0，从机在D0上升沿读取SDA
        SSD1306_W_D0(0); // 拉低D0，主机开始发送下一位数据
    }
}
...
void SSD1306_WriteCommand(uint8_t Command)
{
    SSD1306_W_CS(0);               // 拉低CS，开始通信
    SSD1306_W_DC(0);               // 拉低DC，表示即将发送命令
    SSD1306_SPI_SendByte(Command); // 写入指定命令
    SSD1306_W_CS(1);               // 拉高CS，结束通信
}
...
void SSD1306_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    SSD1306_W_CS(0); // 拉低CS，开始通信
    SSD1306_W_DC(1); // 拉高DC，表示即将发送数据
    /*循环Count次，进行连续的数据写入*/
    for (i = 0; i < Count; i++)
    {
        SSD1306_SPI_SendByte(Data[i]); // 依次发送Data的每一个数据
    }
    SSD1306_W_CS(1); // 拉高CS，结束通信
}
...
void SSD1306_Init(void)
{
    ...
    SSD1306_Update(); // 更新显示，清屏，防止初始化后未显示内容时花屏
}
 ...
 * 返 回 值：无
 * 说    明：SSD1306默认的Y轴，只能8个Bit为一组写入，即1页等于8个Y轴坐标
 */
 ...
 * 返 回 值：等于X的Y次方
 */
uint32_t SSD1306_Pow(uint32_t X, uint32_t Y)
{
 ...
 */
uint8_t SSD1306_pnpoly(uint8_t nvert, int16_t *vertx, int16_t *verty, int16_t testx, int16_t testy)
{
    int16_t i, j, c = 0;
 ...
 */
uint8_t SSD1306_IsInAngle(int16_t X, int16_t Y, int16_t StartAngle, int16_t EndAngle)
{
    int16_t PointAngle;
...
/**
 * 函    数：将SSD1306显存数组更新到SSD1306屏幕
 * 参    数：无
 * 返 回 值：无
 * 说    明：所有的显示函数，都只是对SSD1306显存数组进行读写
 *           随后调用SSD1306_Update函数或SSD1306_UpdateArea函数
 *           才会将显存数组的数据发送到SSD1306硬件，进行显示
...
void SSD1306_Update(void)
{
    ...
    for (j = 0; j < 8; j++)
    {
        ...
        /*连续写入128个数据，将显存数组的数据写入到SSD1306硬件*/
        SSD1306_WriteData(SSD1306_DisplayBuf[j], 128);
    }
}

/**
 * 函    数：将SSD1306显存数组部分更新到SSD1306屏幕
...
void SSD1306_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Page; j < Page1; j++)
    {
        if (X >= 0 && X <= 127 && j >= 0 && j <= 7) // 超出屏幕的内容不显示
        {
            ...
            /*连续写入Width个数据，将显存数组的数据写入到SSD1306硬件*/
...
void SSD1306_Clear(void)
{
    ...
    for (j = 0; j < 8; j++) // 遍历8页
    {
        ...
        {
            SSD1306_DisplayBuf[j][i] = 0x00; // 将显存数组数据全部清零
        }
    }
}

/**
 * 函    数：将SSD1306显存数组部分清零
 * 参    数：X 指定区域左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定区域左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
 * 参    数：Width 指定区域的宽度，范围：0~128
 * 参    数：Height 指定区域的高度，范围：0~64
...
void SSD1306_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Y; j < Y + Height; j++) // 遍历指定页
    {
        for (i = X; i < X + Width; i++) // 遍历指定列
        {
            ...
            {
                SSD1306_DisplayBuf[j / 8][i] &= ~(0x01 << (j % 8)); // 将显存数组指定数据清零
            }
        }
    }
}
...
void SSD1306_Reverse(void)
{
    ...
    for (j = 0; j < 8; j++) // 遍历8页
    {
        for (i = 0; i < 128; i++) // 遍历128列
        {
            SSD1306_DisplayBuf[j][i] ^= 0xFF; // 将显存数组数据全部取反
        }
    }
}

/**
 * 函    数：将SSD1306显存数组部分取反
 * 参    数：X 指定区域左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定区域左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
 * 参    数：Width 指定区域的宽度，范围：0~128
 * 参    数：Height 指定区域的高度，范围：0~64
...
void SSD1306_ReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Y; j < Y + Height; j++) // 遍历指定页
    {
        for (i = X; i < X + Width; i++) // 遍历指定列
        {
            if (i >= 0 && i <= 127 && j >= 0 && j <= 63) // 超出屏幕的内容不显示
            {
                SSD1306_DisplayBuf[j / 8][i] ^= 0x01 << (j % 8); // 将显存数组指定数据取反
            }
        }
    }
}
 ...
 * 参    数：FontSize 指定字体大小
 *           范围：SSD1306_8X16		宽8像素，高16像素
 *                 SSD1306_6X8		宽6像素，高8像素
 * 返 回 值：无
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SSD1306_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
    if (FontSize == SSD1306_8X16) // 字体为宽8像素，高16像素
    {
        /*将ASCII字模库SSD1306_F8x16的指定数据以8*16的图像格式显示*/
        SSD1306_ShowImage(X, Y, 8, 16, SSD1306_F8x16[Char - ' ']);
    }
    else if (FontSize == SSD1306_6X8) // 字体为宽6像素，高8像素
    {
        /*将ASCII字模库SSD1306_F6x8的指定数据以6*8的图像格式显示*/
        SSD1306_ShowImage(X, Y, 6, 8, SSD1306_F6x8[Char - ' ']);
    }
}

/**
 * 函    数：SSD1306显示字符串（支持ASCII码和中文混合写入）
 ...
 *                 SSD1306_6X8		宽6像素，高8像素
 * 返 回 值：无
 * 说    明：显示的中文字符需要在SSD1306_Data.c里的SSD1306_CF16x16数组定义
 *           未找到指定中文字符时，会显示默认图形（一个方框，内部一个问号）
 *           当字体大小为SSD1306_8X16时，中文字符以16*16点阵正常显示
 *           当字体大小为SSD1306_6X8时，中文字符以6*8点阵显示'?'
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SSD1306_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
    ...
    while (String[i] != '\0') // 遍历字符串
    {
        ...
        {
            /*使用SSD1306_ShowChar显示此字符*/
            SSD1306_ShowChar(X + XOffset, Y, SingleChar[0], FontSize);
            XOffset += FontSize;
        }
        else // 否则，即多字节字符
        {
            ...
            for (pIndex = 0; strcmp(SSD1306_CF16x16[pIndex].Index, "") != 0; pIndex++)
            {
                /*找到匹配的字符*/
                if (strcmp(SSD1306_CF16x16[pIndex].Index, SingleChar) == 0)
                {
                    break; // 跳出循环，此时pIndex的值为指定字符的索引
                }
            }
            if (FontSize == SSD1306_8X16) // 给定字体为8*16点阵
            {
                /*将字模库SSD1306_CF16x16的指定数据以16*16的图像格式显示*/
                SSD1306_ShowImage(X + XOffset, Y, 16, 16, SSD1306_CF16x16[pIndex].Data);
                XOffset += 16;
            }
            else if (FontSize == SSD1306_6X8) // 给定字体为6*8点阵
            {
                /*空间不足，此位置显示'?'*/
                SSD1306_ShowChar(X + XOffset, Y, '?', SSD1306_6X8);
                XOffset += SSD1306_6X8;
            }
        }
    }
...
void SSD1306_ShowNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    {
        /*调用SSD1306_ShowChar函数，依次显示每个数字*/
        /*Number / SSD1306_Pow(10, Length - i - 1) % 10 可以十进制提取数字的每一位*/
        /*+ '0' 可将数字转换为字符格式*/
        SSD1306_ShowChar(X + i * FontSize, Y, Number / SSD1306_Pow(10, Length - i - 1) % 10 + '0',
                         FontSize);
    }
}

/**
 * 函    数：SSD1306显示有符号数字（十进制，整数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
...
void SSD1306_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    if (Number >= 0) // 数字大于等于0
    {
        SSD1306_ShowChar(X, Y, '+', FontSize); // 显示+号
    ...
    {
        /*调用SSD1306_ShowChar函数，依次显示每个数字*/
        /*Number1 / SSD1306_Pow(10, Length - i - 1) % 10 可以十进制提取数字的每一位*/
        /*+ '0' 可将数字转换为字符格式*/
        SSD1306_ShowChar(X + (i + 1) * FontSize, Y, Number1 / SSD1306_Pow(10, Length - i - 1) % 10 + '0',
                         FontSize);
    }
}

/**
 * 函    数：SSD1306显示十六进制数字（十六进制，正整数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
...
void SSD1306_ShowHexNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    for (i = 0; i < Length; i++) // 遍历数字的每一位
    {
        ...
        {
            /*调用SSD1306_ShowChar函数，显示此数字*/
            /*+ '0' 可将数字转换为字符格式*/
            SSD1306_ShowChar(X + i * FontSize, Y, SingleNumber + '0', FontSize);
        }
        else // 单个数字大于10
        {
            /*调用SSD1306_ShowChar函数，显示此数字*/
            /*+ 'A' 可将数字转换为从A开始的十六进制字符*/
            SSD1306_ShowChar(X + i * FontSize, Y, SingleNumber - 10 + 'A', FontSize);
        }
    }
}

/**
 * 函    数：SSD1306显示二进制数字（二进制，正整数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
 * 参    数：Number 指定要显示的数字，范围：0x00000000~0xFFFFFFFF
...
void SSD1306_ShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
    ...
    {
        /*调用SSD1306_ShowChar函数，依次显示每个数字*/
        /*Number / SSD1306_Pow(2, Length - i - 1) % 2 可以二进制提取数字的每一位*/
        /*+ '0' 可将数字转换为字符格式*/
        SSD1306_ShowChar(X + i * FontSize, Y, Number / SSD1306_Pow(2, Length - i - 1) % 2 + '0',
                         FontSize);
    }
}

/**
 * 函    数：SSD1306显示浮点数字（十进制，小数）
 * 参    数：X 指定数字左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * 参    数：Y 指定数字左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SSD1306_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength,
                          uint8_t FontSize)
{
    ...
    PowNum = SSD1306_Pow(10, FraLength); // 根据指定小数的位数，确定乘数
    FraNum = round(Number * PowNum);     // 将小数乘到整数，同时四舍五入，避免显示误差
    IntNum += FraNum / PowNum;           // 若四舍五入造成了进位，则需要再加给整数

    /*显示整数部分*/
    SSD1306_ShowNum(X + FontSize, Y, IntNum, IntLength, FontSize);

    /*显示小数点*/
    SSD1306_ShowChar(X + (IntLength + 1) * FontSize, Y, '.', FontSize);

    /*显示小数部分*/
    SSD1306_ShowNum(X + (IntLength + 2) * FontSize, Y, FraNum, FraLength, FontSize);
}
...
/**
 * 函    数：SSD1306使用printf函数打印格式化字符串（支持ASCII码和中文混合写入）
...
void SSD1306_Printf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
    ...
    va_end(arg);                   // 结束变量arg
    // 清除行尾空格
    SSD1306_ShowString(X, Y, String, FontSize); // SSD1306显示字符数组（字符串）
}

/**
 * 函    数：SSD1306在指定位置画一个点
...
void SSD1306_DrawRectangle(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, uint8_t IsFilled)
{
    ...
    if (!IsFilled) // 指定矩形不填充
    {
        ...
        for (i = X; i < X + Width; i++)
        {
            ...
            SSD1306_DrawPoint(i, Y + Height - 1);
        }
        /*遍历左右Y坐标，画矩形左右两条线*/
        for (i = Y; i < Y + Height; i++)
        {
            SSD1306_DrawPoint(X, i);
            SSD1306_DrawPoint(X + Width - 1, i);
        }
    }
    else // 指定矩形填充
    {
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SSD1306_DrawTriangle(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1, int16_t X2, int16_t Y2,
                          uint8_t IsFilled)
{
    ...
    else // 指定三角形填充
    {
        ...
        for (i = minx; i <= maxx; i++)
        {
            ...
            for (j = miny; j <= maxy; j++)
            {
                /*调用SSD1306_pnpoly，判断指定点是否在指定三角形之中*/
                /*如果在，则画点，如果不在，则不做处理*/
...
void SSD1306_DrawCircle(int16_t X, int16_t Y, uint8_t Radius, uint8_t IsFilled)
{
    ...
    /*画每个八分之一圆弧的起始点*/
    SSD1306_DrawPoint(X + x, Y + y);
    SSD1306_DrawPoint(X - x, Y - y);
    SSD1306_DrawPoint(X + y, Y + x);
    SSD1306_DrawPoint(X - y, Y - x);
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void SSD1306_DrawArc(int16_t X, int16_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle,
                     uint8_t IsFilled)
{
    int16_t x, y, d, j;
    ...
    if (SSD1306_IsInAngle(x, y, StartAngle, EndAngle))
    {
        SSD1306_DrawPoint(X + x, Y + y);
    }
    if (SSD1306_IsInAngle(-x, -y, StartAngle, EndAngle))
    ...
    if (SSD1306_IsInAngle(y, x, StartAngle, EndAngle))
    {
        SSD1306_DrawPoint(X + y, Y + x);
    }
    if (SSD1306_IsInAngle(-y, -x, StartAngle, EndAngle))
    ...
    if (IsFilled) // 指定圆弧填充
    {
        ...
        for (j = -y; j < y; j++)
        {
            ...
            if (SSD1306_IsInAngle(0, j, StartAngle, EndAngle))
            {
                SSD1306_DrawPoint(X, Y + j);
            }
        }
    ...
    while (x < y) // 遍历X轴的每个点
    {
        ...
        if (IsFilled) // 指定圆弧填充
        {
            ...
            {
                /*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
                if (SSD1306_IsInAngle(x, j, StartAngle, EndAngle))
                {
                    SSD1306_DrawPoint(X + x, Y + j);
                }
                if (SSD1306_IsInAngle(-x, j, StartAngle, EndAngle))
            ...
            {
                /*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
                if (SSD1306_IsInAngle(-y, j, StartAngle, EndAngle))
                {
                    SSD1306_DrawPoint(X - y, Y + j);
                }
                if (SSD1306_IsInAngle(y, j, StartAngle, EndAngle))
